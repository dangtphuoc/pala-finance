...
import pala.finance.dialog.InputItemDialog;
import pala.finance.dialog.ItemDialog;
import pala.finance.handler.CallBackHandler;
import pala.finance.repository.IncomeItemRepository;
import pala.finance.repository.InputItemRepository;
import pala.finance.repository.ItemRepository;
import pala.finance.repository.ReportService;
import pala.finance.table.ReportByItemTableModel;
import pala.finance.table.ReportByMonthTableModel;
import pala.finance.table.ReportTableModel;
import pala.finance.ui.IncomeType;
import pala.finance.ui.Month;
...
public class MainApp implements CallBackHandler{
	...
	@Autowired
	InputItemRepository inputItemRepo;
	
	@Autowired
	IncomeItemRepository incomeItemRepo;
	
	@Autowired
	ReportService inputRep;
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					ConfigurableApplicationContext applicationContext =  new ClassPathXmlApplicationContext( "/spring/financeContext.xml");
					applicationContext.registerShutdownHook();
					MainApp window = applicationContext.getBean(MainApp.class);//new MainApp();
					window.initialize();
					/*if(splash != null)
						splash.close();*/
	...
	private void loadIncomeTable() {
		EndResult<IncomeItem> items = incomeItemRepo.findAllItems();
	...
	private void loadReportTable(LocalDate fromDate, LocalDate toDate) {
		...
		double totalCost = 0;
		Iterator<InputItem> iter = inputItemRepo.findAllItems().iterator();
		...
		//set total income
		Iterator<IncomeItem> iter1 = incomeItemRepo.findAllItems().iterator();
	...
	private void loadInputItemTable() {
		EndResult<InputItem> items = inputItemRepo.findAllItems();
		String[] columnNames = {"ID", "Input Name", "Cost", "Date"};
		Vector<String> columns = new Vector<String>(Arrays.asList(columnNames));
